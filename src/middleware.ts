...
export async function middleware(request: NextRequest) {
  ...
  // Normalize root (empty string after stripping locale) to "/" so routeMap["/" → public] matches
  const pathnameForRouteCheck = pathnameWithoutLocale || "/"
  const isNotPublic = !isPublicRoute(pathnameForRouteCheck)

  // Handle authentication for protected and guest routes
  if (isNotPublic) {
    const token = await getToken({ req: request })
    const isAuthenticated = !!token
    const isGuest = isGuestRoute(pathnameForRouteCheck)
